...
// Doit rester ≤ à la limite configurée sur le bucket dans Supabase
// (Storage > sema-fichiers > Edit bucket > File size limit) - déjà au
// maximum autorisé par le projet.
...
// Photo de profil — pas liée à une séance, donc pas de vérification de
// groupe nécessaire dans /api/fichier (juste être connecté, eleve ou prof).
export function cheminAvatar(identifiant, nomFichier) {
  const unique = crypto.randomUUID();
  return `avatars/${identifiant}/${unique}-${nomFichier}`;
}
